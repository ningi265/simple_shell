...
char *gets_history_file(info_t *info)
{
	...
	if (!dir)
		return (NULL);
	buf = malloc(sizeof(char) * (_string_length(dir)
	...
	_string_concatenate(buf, HIST_FILE);
	return (buf);
}
...
int writes_history(info_t *info)
{
	...
	{
		_putstringfd(node->str, fd);
		_putcharacterfd('\n', fd);
	}
	_putcharacterfd(BUF_FLUSH, fd);
	close(fd);
	return (1);
}
...
int reads_history(info_t *info)
{
	...
	for (i = 0; i < fsize; i++)
		...
		{
			buf[i] = '\0';
			builds_history_list(info, buf + last, linecount++);
			last = i + 1;
		}
	if (last != i)
		builds_history_list(info, buf + last, linecount++);
...
/**
 * builds_history_list - adds entry to a history linked list
...
int builds_history_list(info_t *info, char *buf, int linecount)
{
	...
	if (info->history)
		node = info->history;
	adds_node_toend(&node, buf, linecount);
