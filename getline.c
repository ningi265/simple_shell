...
#else
		r = _gets_next_line(info, buf, &len_p);
#endif
		if (r > 0)
		{
			if ((*buf)[r - 1] == '\n')
			...
			builds_history_list(info, *buf, info->histcount++);
...
/**
 * _gets_next_line - gets the next line of input from STDIN
 * @info: parameter struct
 ...
 * Return: s
 */
int _gets_next_line(info_t *info, char **ptr, size_t *length)
{
	...
	r = reads_buffer(info, buf, &len);
	if (r == -1 || (r == 0 && len == 0))
		return (-1);

	c = _stringcharacter(buf + i, '\n');
	...
	if (s)
		_stringconcatenate(new_p, buf + i, k - i);
	else
